...
Eigen::Vector2d Node::get_point() { return point; }

size_t Node::get_index() { return index; }
...
Node * Node::Nearest(Eigen::Vector2d point) {
  ...
  if (nearside_best == nullptr ||
      (nearside_best->point - point).norm() >
      fabs(point(dir) - this->point(dir))) {
    if (far_side != nullptr) {
  ...
  Node * best = this;
  if (nearside_best != nullptr &&
      (nearside_best->point-point).norm() < (best->point-point).norm()) {
    best = nearside_best;
  }
  if (farside_best != nullptr &&
      (farside_best->point-point).norm() < (best->point-point).norm()) {
    best = farside_best;
  }
  return best;
}
