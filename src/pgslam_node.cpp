...
void draw_map ()
{
	...
	for (size_t i=0; i<scans.size(); i++) {
		if (max_x < scans[i].get_max_x_in_world()) max_x = scans[i].get_max_x_in_world();
		if (min_x > scans[i].get_min_x_in_world()) min_x = scans[i].get_min_x_in_world();
		if (max_y < scans[i].get_max_y_in_world()) max_y = scans[i].get_max_y_in_world();
		if (min_y > scans[i].get_min_y_in_world()) min_y = scans[i].get_min_y_in_world();
	}
	double max_x_i = (int)((max_x+1.0)*10)/10.0;
	double min_x_i = (int)((min_x-1.0)*10)/10.0;
	double max_y_i = (int)((max_y+1.0)*10)/10.0;
	double min_y_i = (int)((min_y-1.0)*10)/10.0;
	...
	for (int i=0; i<scans.size(); i++) {	// for every scan
		...
		for (int j=1; j<points.size(); j++) {	// for every point
			...
			for (int i=0; i<steps; i++) {	// for every step
				...
				else	emap(current.x(),current.y()) = (int)(emap(current.x(),current.y())*0.8);
				current += step;
			}
			if (v.norm()<draw_range)
				if (emap(current.x(),current.y())==-1)
					...
					emap(current.x(),current.y()) = (int)(emap(current.x(),current.y())*0.2);
...
pgslam::Pose2D ListenPose2D (string target_frame, string source_frame)
{
	pgslam::Pose2D pose;
	// listen
	...
	try {
		...
		plistener->lookupTransform (target_frame, source_frame, Time(0), transform);
...
pgslam::LaserScan RosLaserScan_T_PGSlamLaserScan (const sensor_msgs::LaserScan& msg)
{
	...
	size_t i=0;
	for (double angle=msg.angle_min; angle<=msg.angle_max; angle+=msg.angle_increment, i++) {
